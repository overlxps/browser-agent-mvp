...
FALLBACK_ACTION: dict[Phase, ActionType] = {
    ...
    "searched": ActionType.EXTRACT_PRODUCTS,
...
ALLOWED_ACTIONS: dict[Phase, set[ActionType]] = {
    "start": {ActionType.NAVIGATE},
    "opened": {ActionType.FILL},
    "searched": {ActionType.FILL, ActionType.EXTRACT_PRODUCTS},
    "extracted": {ActionType.FILL, ActionType.FINISH},
}
...
class OpenAICompatiblePlanner:
    ...
    def _fallback(self, phase: Phase, request: RunRequest) -> PlanDecision:
        ...
        reasons = {
            ActionType.NAVIGATE: "打开受信任的本地演示商城",
            ActionType.FILL: f"搜索用户指定的关键词：{request.query}",
            ActionType.EXTRACT_PRODUCTS: "读取页面商品卡片的结构化字段",
            ActionType.FINISH: "完成筛选并返回满足条件的推荐",
        }
        return PlanDecision(Action(type=action_type, reason=reasons[action_type], value=request.query if action_type == ActionType.FILL else None), "fallback")
    ...
    async def next_action(self, phase: Phase, request: RunRequest, observation: str) -> PlanDecision:
        ...
        if not self.settings.api_key:
            return fallback
        prompt = {
            "task": request.model_dump(), "phase": phase, "observation": observation,
        ...
        try:
            async with httpx.AsyncClient(timeout=20) as client:
                response = await client.post(
                    ...
                    json={"model": self.settings.model, "temperature": 0, "response_format": {"type": "json_object"}, "messages": [{"role": "system", "content": "You are a safe browser-task planner."}, {"role": "user", "content": json.dumps(prompt, ensure_ascii=False)}]},
                )
                response.raise_for_status()
                raw = response.json()["choices"][0]["message"]["content"]
                decision = json.loads(raw)
                action = Action(type=decision["type"], reason=decision["reason"], value=request.query if decision["type"] == ActionType.FILL else None)
